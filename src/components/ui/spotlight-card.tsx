...
interface GlowCardProps {
  children: ReactNode;
  className?: string;
  glowColor?: 'blue' | 'purple' | 'green' | 'red' | 'orange' | 'indigo' | 'cyan';
  size?: 'sm' | 'md' | 'lg';
  width?: string | number;
  ...
  const getInlineStyles = () => {
    const baseStyles = {
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 210) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 60) * 1%) / var(--bg-spot-opacity, 0.15)), transparent
      )`,
    ...
    <>
      ...
      <div
        ...
        className={cn(
          ...
          "rounded-2xl relative grid shadow-[0_1rem_3rem_-1rem_rgba(0,0,0,0.5)] p-5 gap-4 backdrop-blur-md border border-white/5",
          className
        )}
      >
